...
public:
	 ...
	 */
	inline void readScene(const char* scenepath);
	 ...
	 */
	inline void getLightFacets();

	/**
	 * @brief write the rendering result to a *.jpg file
	 */
	inline void write2JPEG(const char* filename);

	/**
	* @brief build the relationship point2triangle
	*/
	inline void buildPoint2Triangles();

	/**
	* @brief build grids
	*/
	inline void buildGrids();
...
void Scene::write2JPEG(const char* filename) {
	...
	stbi_write_jpg(filename, camera.width, camera.height, 3, colorData, camera.width*camera.height);
